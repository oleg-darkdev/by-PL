const feasts = [
	{
		title: 'Święto "Święto Pracy"',
		img: ['/cards/feasts/1_maja_pl.svg', '/cards/feasts/1_maja_by.svg'],
		id: 46,
		description: ['', '', ''],
		sourcesPl: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		]
	},
	{
		title: 'Święto "Boże Narodzenie"',
		img: ['/cards/feasts/christmas_pl.svg', '/cards/feasts/christmas_pl.svg'],
		id: 47,
		description: ['', '', ''],
		sourcesPl: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		]
	},
	{
		title: 'Święto "Dożynki"',
		img: ['/cards/feasts/dożynki_pl.svg', '/cards/feasts/dożynki_by.svg'],
		id: 48,
		description: ['', '', ''],
		sourcesPl: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		]
	},
	{
		title: 'Święto "Dziady"',
		img: ['/cards/feasts/dziady_pl.svg', '/cards/feasts/dziady_by.svg'],
		id: 49,
		description: ['', '', ''],
		sourcesPl: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: ',
				link: ''
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			},
			{
				title: 'Wikipedia: ',
				link: ''
			}
		]
	},
	{
		title: '"Kolędy"',
		img: ['/cards/feasts/kolędy_pl.svg', '/cards/feasts/kolędy_by.svg'],
		id: 50,
		description: [
			'Kolęda – pierwotnie radosna pieśń noworoczna, która współcześnie przyjęła powszechnie formę pieśni bożonarodzeniowej (nawiązującej do świąt Bożego Narodzenia). Utrzymywana najczęściej w konwencji religijnej, początkowo wywodząca się z tradycji ludowej, w późniejszym okresie komponowana jest również przez wielu wybitnych kompozytorów. Odmiana kolędy o wątkach zaczerpniętych z życia codziennego nazywana jest pastorałką, która w odróżnieniu od kolędy we współczesnym, potocznym tego słowa rozumieniu, nie jest wykorzystywana w chrześcijańskich nabożeństwach religijnych ze względu na swój świecki charakter[1]. ',
			'Co roku w Polsce odbywają się liczne przeglądy i festiwale kolędowe, z których największym, gromadzącym kilkadziesiąt tysięcy wykonawców jest Ogólnopolski Festiwal Kolęd i Pastorałek w Będzinie.',
			''
		],
		sourcesPl: [
			{
				title: 'Wikipedia: Kolęda',
				link: 'https://pl.wikipedia.org/wiki/Kol%C4%99da'
			},
			{
				title: 'Kolędy kaszubskie',
				link: 'http://pl.kaszubia.com/kultura/jezyk-kaszubski/kaszubskie-koledy/'
			},
			{
				title: 'Kolędy góralskie',
				link: 'http://www.trebunie.pl/teksty_08.htm'
			},
			{
				title: 'Teksty kolęd według archiwalnych śpiewników.',
				link: 'https://wolnelektury.pl/katalog/lektury/koledy/'
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: Каляды',
				link: 'https://be.wikipedia.org/wiki/%D0%9A%D0%B0%D0%BB%D1%8F%D0%B4%D1%8B'
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			},
			{
				title: 'Wikipedia: Коляда',
				link: 'https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D0%BB%D1%8F%D0%B4%D0%B0'
			}
		]
	},
	{
		title: 'Święto "Noc Kupały"',
		img: ['/cards/feasts/kupala_pl.svg', '/cards/feasts/kupala_by.svg'],
		id: 51,
		description: [
			'Noc Kupały (zwana też nocą kupalną, kupalnocką, kupałą) – słowiańskie święto związane z letnim przesileniem Słońca, obchodzone w czasie najkrótszej nocy w roku, co przypada około 21-22 czerwca[1][2]. W krajach anglosaskich pod nazwą Midsummer, w germańskich Mittsommerfest. Kościół katolicki, nie mogąc wykorzenić corocznych obchodów Sobótki z wywodzącej się z wierzeń słowiańskich obyczajowości, podjął próbę zasymilowania święta z obrzędowością chrześcijańską (stąd późniejsza wigilia św. Jana – potocznie zwana też nocą świętojańską, posiadająca wówczas wiele zapożyczeń ze święta wcześniejszego – obchodzona w nocy z 23 na 24 czerwca[1][2]). ',
			'Na południu Polski, Podkarpaciu i Śląsku uroczystość o podobnym charakterze zwie się sobótką lub sobótkami, na Warmii i Mazurach palinocką. Nazwa Noc Kupały, kupalnocka utrwaliła się w tradycji Mazowsza i Podlasia. ',
			'Święto ognia, wody, słońca i księżyca, urodzaju, płodności, radości i miłości, powszechnie obchodzone na obszarach zamieszkiwanych przez ludy słowiańskie, ale również w podobnym charakterze na obszarach zamieszkiwanych przez ludy bałtyckie, germańskie i celtyckie, a także przez część narodów ugrofińskich, np. Finów (w Finlandii jest jednym z najważniejszych świąt w kalendarzu) i Estończyków. Na Łotwie pod nazwą Līgo jest wolnym od pracy świętem państwowym (23 i 24 czerwca). Również na Litwie dzień 24 czerwca, od 2005 roku, jest wolny od pracy.',
			'Święto to jest często opisywane i popularyzowane współcześnie jako rodzima alternatywa wobec zachodnich walentynek[4].'
		],
		sourcesPl: [
			{
				title: 'Wikipedia: Noc Kupały',
				link: 'https://pl.wikipedia.org/wiki/Noc_Kupa%C5%82y'
			},
			{
				title: 'Vogue: Noc Kupały – magia pożądania ',
				link: 'https://www.vogue.pl/a/noc-kupaly-znaczenie-symbolika-obrzedy'
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: Иван Купала',
				link: 'https://ru.wikipedia.org/wiki/%D0%98%D0%B2%D0%B0%D0%BD_%D0%9A%D1%83%D0%BF%D0%B0%D0%BB%D0%B0'
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			},
			{
				title: 'Wikipedia: Купалле',
				link: 'https://be.wikipedia.org/wiki/%D0%9A%D1%83%D0%BF%D0%B0%D0%BB%D0%BB%D0%B5'
			},
			{
				title: 'Wikipedia: Ночь Купала',
				link: 'https://wiki5.ru/wiki/Kupala_Night'
			}
		]
	},
	{
		title: 'Święto "Walentynki"',
		img: ['/cards/feasts/walentynki_pl.svg', '/cards/feasts/walentynki_by.svg'],
		id: 52,
		description: [
			'Coroczne święto zakochanych przypadające 14 lutego. Nazwa pochodzi od św. Walentego, którego wspomnienie liturgiczne w Kościele katolickim obchodzone jest również tego dnia. ',
			'Walentynki są obchodzone w południowej i zachodniej Europie od średniowiecza. Europa północna i wschodnia dołączyła do walentynkowego grona znacznie później[potrzebny przypis]. ',
			'Pomimo katolickiego patrona tego święta, czasem wiązane jest ono ze zbieżnym terminowo zwyczajem pochodzącym z Cesarstwa Rzymskiego, polegającym głównie na poszukiwaniu wybranki serca, np. przez losowanie jej imienia ze specjalnej urny. Współczesny dzień zakochanych nie ma jednak bezpośredniego związku z jednym konkretnym świętem starożytnego Rzymu, choć jest kojarzony z takimi postaciami z mitologii jak Kupidyn, Eros, Pan czy Juno Februata.'
		],
		sourcesPl: [
			{
				title: 'Wikipedia: Walentynki',
				link: 'https://pl.wikipedia.org/wiki/Walentynki'
			},
			{
				title: 'Walentynki w Polsce i na świecie - historia i ciekawostki',
				link: 'https://www.garneczki.pl/blog/walentynki-historia-i-ciekawostki/'
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: День святого Валентина',
				link: 'https://ru.wikipedia.org/wiki/%D0%94%D0%B5%D0%BD%D1%8C_%D1%81%D0%B2%D1%8F%D1%82%D0%BE%D0%B3%D0%BE_%D0%92%D0%B0%D0%BB%D0%B5%D0%BD%D1%82%D0%B8%D0%BD%D0%B0'
			}
		]
	},
	{
		title: 'Święto "Wielkanoc"',
		img: ['/cards/feasts/wielkanoc_pl.svg', '/cards/feasts/wielkanoc_by.svg'],
		id: 53,
		description: [
			'Wielkanoc w Polsce jest jednym z najważniejszych świąt w Polsce, często porównywalnym pod względem znaczenia kulturowego do Bożego Narodzenia. Jest to święto państwowe, wolne od pracy. Wiąże się z nim wiele specyficznych zwyczajów i tradycji[1][2]. ',
			'Wielkanoc obchodzona jest w Polsce od chrystianizacji kraju w średniowieczu[2]. W dobie rozbiorów było ono postrzegane jako ważne święto patriotyczne, przypominające Polakom o ich kulturze.',
			'Niektóre z polskich tradycji wielkanocnych mają swoje korzenie we wcześniejszych wierzeniach pogańskich, a inne były fuzją tych tradycji z nową tradycją chrześcijańską. Do takich tradycji zaliczają się między innymi te związane ze święconką, przygotowywaną w Wielką Sobotę i pisankami. W czasie Wielkanocy praca, w tym prace domowe, powinna być ograniczona do minimum. Ponieważ Polska jest w przeważającej części katolicka, większość tradycji wiąże się z Kościołem katolickim, chociaż wiele z innych mniejszości religijnych w Polsce również ma swoje zwyczaje związane z Wielkanocą.'
		],
		sourcesPl: [
			{
				title: 'Wikipedia: Wielkanoc',
				link: 'https://pl.wikipedia.org/wiki/Wielkanoc'
			},
			{
				title: 'Wikipedia: Wielkanoc w Polsce',
				link: 'https://pl.wikipedia.org/wiki/Wielkanoc_w_Polsce'
			},
			{
				title: 'Kowalczyk, Maria (2020). "Geneza i obchód Wielkanocy w Polsce. Zarys problematyki"',
				link: 'studiaelblaskie.pl/assets/Artykuly/CB-16-A19-Kowalczyk-Rew-1.pdf'
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: Пасха',
				link: 'https://ru.wikipedia.org/wiki/%D0%9F%D0%B0%D1%81%D1%85%D0%B0'
			},
			{
				title: 'Wikipedia: Обряды и обычаи белорусов',
				link: 'https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%80%D1%8F%D0%B4%D1%8B_%D0%B8_%D0%BE%D0%B1%D1%8B%D1%87%D0%B0%D0%B8_%D0%B1%D0%B5%D0%BB%D0%BE%D1%80%D1%83%D1%81%D0%BE%D0%B2'
			}
		]
	},
	{
		title: 'Święto "Dzień Kobiet"',
		img: ['/cards/feasts/womens_day_pl.svg', '/cards/feasts/womens_day_by.svg'],
		id: 54,
		description: [
			'W Polsce święto było popularne w okresie PRL. W latach 70. do popularnych podarunków dołączyły kwiaty. Początkowo ofiarowywano goździki i rajstopy, a później tulipany. Z okazji tego dnia publikowano portrety kobiet różnych sektorów życia gospodarczego zgodnie z opinią Władysława Gomułki, że „nie ma dziś w Polsce dziedziny, w której kobiety nie odgrywałyby ważnej roli”. W zakładach pracy czy szkołach był obchodzony obowiązkowo. Była to okazja do uzupełniania braków w zaopatrzeniu, dlatego wręczano paniom takie dobra materialne jak rajstopy, ręcznik, ścierka, mydło, kawa.',
			'Dzień ten, podobnie jak inne uroczystości: Dzień Czynu Partyjnego, Dzień Hutnika, Dzień Drukarza, Dzień Pracownika Przemysłu Spożywczego – według niektórych istniał po to, aby obywatele nie zapomnieli o „przywiązaniu do Państwa Ludowego, miłości do wielkich idei pokoju i socjalizmu”',
			'Obecnie corocznie odbywają się organizowane przez feministki manify, czyli demonstracje połączone z happeningiem na rzecz równego traktowania kobiet i mężczyzn. Prezentami są najczęściej kwiaty lub słodycze. '
		],
		sourcesPl: [
			{
				title: 'Wikipedia: Dzień Kobiet',
				link: 'https://pl.wikipedia.org/wiki/Dzie%C5%84_Kobiet'
			},
			{
				title: "About International Women's Day",
				link: 'https://www.internationalwomensday.com/About'
			},
			{
				title: 'International womens day history',
				link: 'https://web.archive.org/web/20160312120336/http://womenwatch.unwomen.org/international-womens-day-history'
			}
		],
		sourcesRu: [
			{
				title: 'Wikipedia: Международный женский день',
				link: 'https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D0%B6%D0%B4%D1%83%D0%BD%D0%B0%D1%80%D0%BE%D0%B4%D0%BD%D1%8B%D0%B9_%D0%B6%D0%B5%D0%BD%D1%81%D0%BA%D0%B8%D0%B9_%D0%B4%D0%B5%D0%BD%D1%8C'
			}
		]
	}
];



export default feasts;

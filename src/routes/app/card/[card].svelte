<script>
	...
	// import { Breadcrumb, Breadc/rumbItem } from 'flowbite-svelte';

	const cardsList = [].concat(
			boardgameCards.history,
			boardgameCards.culture,
			boardgameCards.territory,
			boardgameCards.feasts
		),
		cardData = cardsList[$page.params.card - 1];
</script>

<main class="flex items-center p-2 flex-col pt-20 mb-8">
		...
		<div class="flex flex-row flex-wrap  justify-center">
			...
			>
				...
				<div class="p-4 sm:p-6">
					<a
						href="#sources-en"
						class="text-center font-medium focus:ring-4 focus:outline-none inline-flex items-center justify-center px-6 py-3.5 text-base text-white bg-gray-900 hover:bg-gray-700 focus:ring-blue-300  rounded-lg"
					>
		...
		<article class="mb-10 w-full p-4">
			...
			{#each cardData.sourcesPl as sourceData}
				<ol id="sources-en" class="gradient-list">
					<li>
						<a href={sourceData.link} class="inline-flex items-center text-gray-700 hover:underline"
							>{sourceData.title}
							<svg
